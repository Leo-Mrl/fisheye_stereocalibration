...
std::shared_ptr<LensFishEye> calibration(const int gridwidth, const int gridheight, 
    ...
    for (size_t idimg = 0; idimg < images.size(); idimg++) {
        ...
        if (lens == nullptr) {
            ...
            lens = std::shared_ptr<LensFishEye>(new LensFishEye(width, height, focal, principal_point, distortion));
        }

        /*Extract corners from image*/
        if (!cextractor.extract(img, lens, grid)) {
            std::cout << "Failed to detect checkerboard on this image : " << images[idimg] << std::endl;
            return fail;
        }

        //std::cout << "Checkerboard detected." << std::endl;
    ...
    if (iest.estimate(grid, views) == false) {
        ...
        printf("\nEstimation of intrinsic parameters succeeded, focal length in u : %.2f", lens->getFocalU());
    }
    ...
    int pos = 0;
    for (std::vector<std::shared_ptr<Camera> >::iterator it = views.begin(); it != views.end(); it++) {
...
cv::Mat cameraMatrixforEG(std::vector<std::string> images, std::shared_ptr<LensFishEye> myLens, 
    ...
    for (int i=0; i<fabs(images.size()); i++){
        ...
        bool found = cv::findChessboardCorners(sharp, boardSize, cornersTemp, cv::CALIB_CB_ADAPTIVE_THRESH + cv::CALIB_CB_FILTER_QUADS + cv::CALIB_CB_NORMALIZE_IMAGE);
...
void extrinsicEstimation(const std::vector<cv::Mat> &RotationsL, const std::vector<cv::Mat> &RotationsR, 
    ...
    float distCam = sqrt(pow(trFinal.at<double>(0,0),2) + pow(trFinal.at<double>(1,0),2) + pow(trFinal.at<double>(2,0),2));
